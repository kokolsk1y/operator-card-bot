...
Я собираю карточку товара. Жми /new и иди по шагам:
1️⃣ *Название* — станет заголовком по центру
2️⃣ *Бейдж* в рамке — по желанию (или «Пропустить»)
3️⃣ *Фото* товара — фон уберу сам, *в фоне* (можешь сразу писать дальше)
4️⃣ *До 4 характеристик* (или «Пропустить» все)
5️⃣ *«Собрать карточку»* — пришлю готовый PNG

✏️ *Текст в две строки* (крупное значение + мелкая подпись):
раздели знаком \`^\` или с новой строки —
• \`1,5 ^ метра\`
• \`ПВС ^ 3х1 мм\`
• \`10А ^ нагрузка ^ *до 2300Вт\` (3-я часть — мелкое примечание)
Без \`^\` — одна строка: \`3 розетки\`

⚠️ *Важно:*
• Каждая часть — не длиннее \`20\` символов, иначе не влезет.
• Если характеристик *нет* — товар встанет *по центру* карточки.
• Знак «/» больше *не* разделяет — пиши «16/250 В» как есть.

📷 *Фото:*
• Лучше всего — товар на простом светлом фоне.
• Можно прислать как фото или как файл (png/jpg/webp).
• Не нравится вырез — жми «🔁 Переснять фото».

🛠 *После готовой карточки* (кнопки под ней):
• 🔄 Заголовок · 🖼 Фото · 📝 Характеристики — поправить
• 📐 Подвинуть — сдвинуть любой элемент стрелками
• 🆕 Новая карточка — начать заново

Команды: /new — новая · /help — эта справка`;
...
bot.start({ onStart: async (i) => {
  ...
  console.log(`✅ Бот @${i.username} запущен. Ctrl+C — стоп.`);
} });
